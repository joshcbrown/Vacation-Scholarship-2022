...
class Kernel(abc.ABC, nn.Module):
    ...
    def __init__(self, bandwidth=0.05, dimensions=1):
...
class MultivariateGaussianKernel(Kernel):
    # TODO: normalise. for our purposes of estimation of grad_x log p(x), 
    # this kernel works fine but it is not actually modelling a pdf.
    def forward(self, test_Xs, train_Xs):
        diffs = self._diffs(test_Xs, train_Xs)
        constant = ((2 * np.pi) ** (-self.dims / 2)) * (torch.det(self.bandwidth) ** -1/2)
        exp = torch.exp(-1/2 * (diffs @ (self.bandwidth.inverse()) * diffs).sum(-1))
        return (constant * exp).mean(1)
    def sample(self, train_Xs):
        pass
...
class KernelDensityEstimator(GenerativeModel):
    ...
    def __init__(self, train_Xs, dims, bandwidth=None):
        ...
        if bandwidth is not None:
            ...
            # silverman's rule of thumb from wikipedia https://www.youtube.com/watch?v=FTqs9hiMxPw&t=261s
            n = train_Xs.shape[0]
            sqrt_h = (4/(dims+2)) ** (1/(dims+4)) * n ** (-1/(dims+4)) * train_Xs.std(0) 
            self.bandwidth = sqrt_h ** 2 * torch.eye(dims)
        self.kernel = MultivariateGaussianKernel(self.bandwidth, dims)
